...
class IntListSpec extends UnitSpec {
  ...
  "IntList operations allow to" - {
    ...
    "Execute a function for each value" in {
      var compteur = 0
      def compte(i: Int) = { compteur = compteur + i}

      IntList(1, 2, 3).foreach( x => compte(x) )
      compteur should === (6)


      IntList(1, 2, 3).foreach(x => println(x))
    }
    ...
    "Product the values" in {
      IntList().product should === (1)
      IntList(5).product should === (5)
      IntList(2, 3, 4).product should === (24)
    }
